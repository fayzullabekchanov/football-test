...
export default function MainTest() {
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      secondsRef.current++
      if (secondsRef.current >= 60) { minutesRef.current++; secondsRef.current = 0 }
  ...
  const handleAnswer = (ball) => {
    ...
    const elapsed = minutesRef.current * 60 + secondsRef.current
    const newScores = [...scores]; newScores[currentGroup] = ball
    const newTimes = [...times]; newTimes[currentGroup] = elapsed
    setScores(newScores); setTimes(newTimes)
    setTimeout(() => {
      if (currentGroup < 2) { setCurrentGroup(currentGroup + 1); setAnswered(false) }
      else { finishTest(newScores, newTimes) }
    }, 400)
  }
  ...
  const finishTest = async (finalScores, finalTimes) => {
    ...
    const payload = { bal1: String(bal1), bal2: String(bal2), bal3: String(bal3), totalBall: String(totalBall), anstime1, anstime2, anstime3, totalAnstime }
    setResult(payload)
    try {
  ...
  if (status === 'loading') return <div className="spinner" style={{ minHeight: '100vh', alignItems: 'center' }}><div className="spinner-ring"></div></div>
      ...
      <div style={{ minHeight: '100vh', background: '#f0f4f1', padding: '2rem 1rem' }}>
        <div className="test-container">
          <div className="done-screen">
            ...
            <h2>{t.test_done_title}</h2>
            {submitting ? <p>{t.test_saving}</p> : result ? (
              <>
                <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap', margin: '1.5rem 0' }}>
                  {[1,2,3].map(i => (
                    <div key={i} style={{ background: '#f0f9f3', border: '2px solid #22883c', borderRadius: 12, padding: '1rem 1.5rem', minWidth: 120 }}>
                      <div style={{ fontSize: '0.75rem', color: '#666', marginBottom: '0.25rem' }}>{i}{t.test_ball_label}</div>
                ...
                <div className="score-badge">{t.test_total} {result.totalBall} {t.test_total_ball}</div>
                <p style={{ marginTop: '1rem', color: '#888', fontSize: '0.85rem' }}>{t.test_total_time} {result.totalAnstime}</p>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f0f4f1' }}>
      ...
      <div className="test-container">
        <div className="progress-text">{currentGroup + 1} {t.test_of}</div>
        <div className="progress-bar-wrap">
          <div className="progress-bar-fill" style={{ width: `${((currentGroup + 1) / 3) * 100}%` }}></div>
        </div>
        <div className="question-card" key={currentGroup}>
          <div className="question-header" style={{ background: colors[currentGroup] }}>
            <div className="question-number">{currentGroup + 1}{t.test_situation}</div>
            <h2>{t.test_question}</h2>
          </div>
          <div className="question-body">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={`/images/savollar/${group.id}.png`} alt={`Vaziyat ${group.id}`}
              className="question-img" onError={e => { e.target.style.display = 'none' }} />
          </div>
          <div className="question-footer">
            <div className="answer-group">
              {group.answers.map((ans, idx) => (
                <button key={idx} className="answer-btn" onClick={() => handleAnswer(ans.ball)}
                  disabled={answered} style={{ opacity: answered ? 0.6 : 1 }}>
                  {ans.label}
                </button>
              ))}
            </div>
